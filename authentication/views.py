...
class RegisterView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        # frontend = 'https://village.ng/verify-email'

        relativeLink = reverse('authentications:email-verify')
        absurl='http://'+ current_site+relativeLink+"?token="+str(token)
        # absurl = frontend + "?token=" + str(token)
        email_body = 'Hi ' + user.contact_person + ' thank you for registering with us at onestack\nuse the link below to verify your email \n' + absurl
...
class RequestPasswordResetEmail(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        if User.objects.filter(email=email).exists():
            ...
            # frontend = 'village.ng/reset-confirm'
            absurl = 'http://'+current_site + relativeLink + uidb64 + token
